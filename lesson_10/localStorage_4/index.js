...
function search() {
  ...
  const newBooksArray = lsBooks.filter((book) =>
...
function clearBookFromLs() {
  localStorage.removeItem("books");
}
function draw(bd) {
  clearBooks();
  const booksData = bd.filter((book) =>
    book.title.toLowerCase().includes(inputSearchValueGlobal.toLowerCase())
  );
