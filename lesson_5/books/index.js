...
function getBooksByCountry(country) {
    var result = [];
    for (let index = 0; index < books.length; index++) {
        var singleBook = books[index]
        if (singleBook.country.toLowerCase() === country.toLowerCase()) {
            result.push(singleBook)
        }
    }
    return result;
}
// filter same like function getBooksByCountry
function getBooksByCountryNG(country) {
    var result = books.filter(function (currentBook) {
        return currentBook.country.toLowerCase() === country.toLowerCase()
    })
    return result
}

function getBooksByCountryNG2(country) {
    return books.filter((currentBook) => currentBook.country.toLowerCase() === country.toLowerCase())
}
// arrow function
function regularFn(a) { return a + 1 }
